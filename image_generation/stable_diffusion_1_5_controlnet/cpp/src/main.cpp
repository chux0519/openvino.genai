...
StableDiffusionModels compile_models(const std::string& model_path, const std::string& device, const bool use_cache) {
    ...
    // Tokenizer
    {
        ...
        models.tokenizer = core.compile_model(model_path + "/ov_tokenizer/openvino_tokenizer.xml", "CPU");
    }
...
int32_t main(int32_t argc, char* argv[]) try {
    ...
    options.add_options()(
        ...
        cxxopts::value<std::string>()->default_value("pope francis, best quality, extremely detailed"))(
        "n,negPrompt",
        "Defaut is empty with space",
        cxxopts::value<std::string>()->default_value("monochrome, lowres, bad anatomy, worst quality, low quality"))(
        "d,device",
        ...
        cxxopts::value<std::string>()->default_value("D:\\ov_models\\sd15_openpose_202401_batch_2"))(
        "t,type",
    ...
    StableDiffusionModels models = compile_models(model_path, device, true);

    for (auto input : models.unet.inputs()) {
        std::cout << "any name: " << input.get_any_name() << std::endl;
        for (auto name : input.get_names()) {
            std::cout << "names: " << name << std::endl;
        }
    }

    auto mid_block_node = models.unet.input("mid_block_additional_residual");
    for (auto name : mid_block_node.get_names()) {
        std::cout << "mid_block_additional_residual names: " << name << std::endl;
    }
    std::cout << "mid_block_node get_partial_shape: " << mid_block_node.get_partial_shape() << std::endl;
    return 0;
    ...
    std::cout << "sample_shape: " << sample_shape.size() << std::endl;
    // OPENVINO_ASSERT(sample_shape.is_dynamic() ||
    //                     (sample_shape[2] * VAE_SCALE_FACTOR == height && sample_shape[3] * VAE_SCALE_FACTOR ==
    //                     width),
    ...
    for (uint32_t n = 0; n < num_images; n++) {
        ...
        for (size_t inference_step = 0; inference_step < num_inference_steps; inference_step++) {
            ...
            std::cout << "TODO: should call controlnet" << std::endl;

            // TODO: controlnet
            // image is a tendor, resized and pad to 512x512(peserving original aspect ratio)
            ...
            // *down_and_mid_blok_samples])[self.unet_out]
